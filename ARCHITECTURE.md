...
│     └── Preferences
│
├── 🛒 Marketplace  app/user/marketplace/page.tsx
│  ├── Two tabs: RevRa Pool | My Workspace
│  ├── Lead card grid (tier badge, price, custom fields)
│  ├── "Buy Lead" → Stripe Checkout → added to pipeline
...
│
├── 🛒 Marketplace  app/admin/marketplace/page.tsx
│  ├── Stripe Connect onboarding
│  ├── Workspace lead pool management (list, edit, remove)
│  ├── Tier price editor (workspace custom prices)
│  └── Revenue tracking
│
...
│
├── 🛒 Marketplace  app/superadmin/marketplace/page.tsx
│  ├── RevRa platform pool management
│  ├── Lead upload (single + custom fields)
│  ├── Tier price editor ($50/$25/$15 defaults)
│  └── Platform revenue / sales stats
...
└── ✅ Lead Marketplace  (RevRa + workspace pools)
```
...
│
├── RevRa Pool (superadmin)
│  ├── GET  /api/marketplace/superadmin/leads  — list available RevRa leads
│  ├── POST /api/marketplace/superadmin/leads  — upload lead to RevRa pool
│  ├── PATCH /api/marketplace/superadmin/leads/[id] — edit RevRa lead
│  ├── DELETE /api/marketplace/superadmin/leads/[id] — remove RevRa lead
│  ├── GET  /api/marketplace/superadmin/tiers  — RevRa tier prices
│  └── PATCH /api/marketplace/superadmin/tiers — update RevRa tier prices
│
├── Workspace Pool (admin)
│  ├── GET  /api/marketplace/admin/leads  — list workspace leads
│  ├── POST /api/marketplace/admin/leads  — list workspace lead (requires Stripe)
│  ├── PATCH /api/marketplace/admin/leads/[id] — edit workspace lead
│  ├── DELETE /api/marketplace/admin/leads/[id] — remove workspace lead
│  ├── GET  /api/marketplace/admin/tiers  — workspace tier prices
│  ├── PATCH /api/marketplace/admin/tiers — update workspace tier prices
│  ├── GET  /api/marketplace/admin/stripe-status — check Stripe Connect status
│  └── POST /api/marketplace/admin/stripe-connect — start Stripe Connect OAuth
│
├── Checkout
│  └── POST /api/marketplace/checkout — create Stripe Checkout session
│
└── Purchased
  ├── GET  /api/marketplace/workspace/purchased — list purchased leads
  └── POST /api/webhooks/stripe — payment success → create CRM lead
...
│ Stripe Payments  │ Stripe  │ ✅ Built  │ STRIPE_*  │
...
│ Lead Marketplace  │ Stripe + Supabase│ ✅ Built  │ —  │
│ Bulk Upload (Marketplace) │ CSV import  │ ✅ Built  │ —  │
└────────────────────────────┴──────────────────┴────────────────┴────────────┘
...
│  ④ Admin payout via Stripe Connect      │
...
│   ├── team/page.tsx
│   └── texts/page.tsx
│  ├── marketplace/page.tsx  (RevRa + workspace pool)
│  ├── settings/page.tsx
│  ├── tasks/page.tsx
│  ├── team/page.tsx
│  └── texts/page.tsx
...
│  │  ├── marketplace/page.tsx  (workspace lead sales + Stripe)
...
│  ├── marketplace/page.tsx  (RevRa pool + tier prices)
    └── settings/page.tsx
```
...
| ✅ Complete | Lead Marketplace (RevRa + workspace pools) | Built |
